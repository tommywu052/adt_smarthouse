            ...
            if (client != null)
            {
                try
                {
                    if (eventGridEvent != null && eventGridEvent.Data != null)
                    {
                        ...
                        var parentId = await AdtUtilities.FindParentByQueryAsync(client, twinId, log);

                        if (parentId != null)
                        {
                            log.LogWarning("==============================================");
                            log.LogWarning("ParentID: " + parentId);
                            ...
                            foreach (var operation in message["data"]["patch"])
                            {
                                ...
                                if (opValue.Equals("replace"))
                                {
                                    ...
                                    log.LogWarning("Path: " + propertyPath);

                                    if (propertyPath.Equals("/PIRSensor/PIRState"))
                                    {
                                        await AdtUtilities.UpdateTwinPropertyAsync(client, parentId, propertyPath, operation["value"].Value<bool>(), log);
                                    }
                                    else if (propertyPath.Equals("/DistanceSensor/Distance"))
                                    {
                                        await AdtUtilities.UpdateTwinPropertyAsync(client, parentId, propertyPath, operation["value"].Value<float>(), log);
                                    }
                                    else if (propertyPath.Equals("/label"))
                                    {
